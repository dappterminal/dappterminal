...
/**
 * supply command — supply assets to Aave v3
 */
export const supplyCommand: Command = {
  id: 'supply',
  scope: 'G_p',
  protocol: 'aave-v3',
  description: 'Supply assets to Aave v3',
  aliases: ['deposit', 'lend'],

  async run(args: unknown, context: ExecutionContext): Promise<CommandResult<AaveV3SupplyRequestData>> {
    try {
      // Parse arguments: supply <amount> <asset> [--collateral]
      const { amount, asset, collateral } = parseSupplyArgs(args)

      if (!amount || !asset) {
        return {
          success: false,
          error: new Error('Usage: supply <amount> <asset> [--collateral]\n  Example: supply 100 usdc --collateral'),
        }
      }
      ...
      // Token addresses for supported assets on each chain
      ...
      // Return supply request data for handler
      const supplyData: AaveV3SupplyRequestData = {
        ...
        displayData: {
          asset: assetUpper,
          amount,
          collateral: collateral ? 'Enabled' : 'Disabled',
        },
      }
...
function parseSupplyArgs(args: unknown): { amount?: string; asset?: string; collateral: boolean } {
  const tokens = tokenizeArgs(args)
  let amount: string | undefined
  let asset: string | undefined
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    // First non-flag token is amount
    if (!amount) {
      amount = token
      continue
    }
  ...
  return { amount, asset, collateral }
}
